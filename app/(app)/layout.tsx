...
export default async function AppLayout({ children }: AppLayoutProps) {
  ...
  const displayJobTitle = profile.job_title?.trim() || displayRole;
  ...
  <div className="brand-text">
    ...
    <div className="brand-subtitle">
      Productivity suite
        ...
        <SidebarNav role={profile.role} unread={notificationCount} />
          ...
          <header className="user-header">
            <div className="user-header-left">
              ...
              <div className="user-header-details">
                <span className="user-header-label">Welcome back</span>
                <strong className="user-header-name">{profile.full_name}</strong>
                <div className="user-header-meta">
                  <span>{displayJobTitle}</span>
                  {displayJobTitle.toLowerCase() !== displayRole.toLowerCase() && (
                    <>
                      <span className="user-header-divider" aria-hidden="true">•</span>
                      <span>{displayRole}</span>
                    </>
                  )}
                </div>
            ...
            <div className="user-header-right">
              ...
              >
                ...
                {notificationCount > 0 && (
                  <span className="notification-count">{Math.min(notificationCount, 99)}</span>
                )}
              </Link>
